...
The R GUI has its own basic text editing window for script files. There is a menu item to open the window with a blank space, as well s the standard item to open the script file.
...
We will download the `installr` package now. Downloading is the first part of installing a package. This just puts it within the R framework for later use. It odes not mean you have immediate access to the package functionality.
...
Then we need to look for a CRAN mirror. This is one of the servers around the owrld that holds the R packages. There is a menu item, but I use the command:

`chooseCRANmirror(ind=1`

which selects the first item off that list. It is the cloud-based server operated by RStudio. This is a redirection service and helps find hte closest server to us.
