...
5. Type in the name of the project (e.g. "Lab1") and select the folder you want it to be in (by default it will go into your "Documents" folder).
...
Now, go and shut down RStudio, then navigate to `Documents/Lab1` in the Windows Explorer and open the `Lab1.Rproj` file. RStudio will re-start and you'll be back where you were.
...
2. Now type (or copy/paste) the following into the RScript file.
    ...
    ```
    #' 227.215 Biostats Lab 1.
    
    #' In this lab we'll take another look at data on Moroccan donkeys that we looked at
    #' in semester 1.
...
6. Now we'll produce an R notebook. Before we do this we'll need to make sure the machine is setup properly (by default, unfortunately the Massey computers are not setup correctly) - you'll only need do this once! Copy and paste the following into the `Console` window.
```
install.packages("rmarkdown", "knitr")
```

7. Right, we're now ready to go. Click on the little spiral-bound notebook on the toolbar (it will say `Compile Notebook` when you hover over it). When prompted, select `HTML` as the type of notebook to compile. It will do it's thing, and produce an HTML document that should look something like what is below.
...
plot(c(0,dim(fig)[2]), c(0,dim(fig)[1]), type="n", xaxt="n", yaxt="n",xlab="", ylab="", xaxs="i", yaxs="i", bty="n")
rasterImage(fig, 0, 0, dim(fig)[2], dim(fig)[1])
```

8. Try re-building the notebook and this time selecting `Word` as the type of notebook. It will generate a word document for you :)
