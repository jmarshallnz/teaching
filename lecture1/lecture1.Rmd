...
#  annotate('text', x=(91+135)/2, y=ab[1] + ab[2]*90, label="Variation in heart girth", vjust=-0.3) +
  ...
  annotate('text', x=95, y=160, label = expression(atop("Variation in body weight", "around the mean (unexplained)")), hjust=1) +
...
```{r}
# TODO: Have a graph here showing what x_i, y_i, epsilon_i, \hat{y}_i are.
# As the line represents the average of y, it makes sense that the average of the residuals
# is zero. We also want the best line: the best one will explain the most variation in y, so have
# the least variation left over. Thus, we minimise the variance of the residuals.
# this is the same as minimising the sum of squared residuals (SS_res)
#
```
...
<iframe src="http://shiny.massey.ac.nz/jcmarsha/leastsquares/" style="border: none"></iframe>
...
&= \frac{\sigma_\mathsf{Y}^2 - \sigma_\mathsf{res}^2}{\sigma_\mathsf{Y}^2} = \frac{SS_\mathsf{total} - SS_\mathsf{res}}{SS_\mathsf{total}}
\end{aligned}
$$
...
<iframe src="http://shiny.massey.ac.nz/jcmarsha/rsquared/" style="border: none"></iframe>
...
<iframe src="http://shiny.massey.ac.nz/jcmarsha/linearity/" style="border: none"></iframe>
...
<iframe src="http://shiny.massey.ac.nz/jcmarsha/influence/" style="border: none"></iframe>
