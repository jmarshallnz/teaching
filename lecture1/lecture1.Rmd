...
- We minimise the variance of the residuals
$$
\mathsf{var}(\epsilon_i) = \frac{1}{n}\sum_{i=1}^n (y_i - (\alpha + \beta x_i))^2
$$
...
- The `Estimate` column gives us the values of our coefficients.
    - The estimate for `Heartgirth` means a 1 unit increase in heart girth corresponds to a `r round(coef(mod)[2],2)` unit increase in body weight.
...
$$
R^2 = \frac{\sigma_\mathsf{Y}^2 - \sigma_\mathsf{res}^2}{\sigma_\mathsf{Y}^2}
$$
...
- We can predict the average of $y$ for any given value of $x$ by just substituting values into the estimated linear model equation.
...
- More than one measurement is related to body weight.
- We should model average body weight conditional on all the variables.
- That way we explain more variation and get more precise predictions.
...
```{r, comment="", echo=FALSE}
mod <- lm(Bodywt ~ Heartgirth + Height, data=donkey)
summary(mod)
```
