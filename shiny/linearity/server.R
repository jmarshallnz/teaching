...
get_data = function(curve=0, hetero=0, r) {
  ...
  y = 0.5*x^(abs(curve)+1) + r + 0.5*(exp(r)*x)^(abs(curve)+1)*hetero
  if (curve < 0) {
    m = lm(y ~ x)
    y = fitted(m)-resid(m)
  }
...
shinyServer(function(input, output, session) {
  ...
  output$resid <- renderPlot({
    ...
    r = resid(val()$m)
    yr = max(max(r), -min(r))
    plot(fitted(val()$m), r, ylim=c(-yr, yr), pch=19, col="#00000050", xaxt="n", yaxt="n", xlab="Fitted value", ylab="Residuals")
    abline(h=0, lty="dotted")
    text(max(fitted(val()$m)), -yr, "Residuals", col="#00000040", adj=c(1,0))
  })
})
