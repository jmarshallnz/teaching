...
## Exercise 5: Remember to Vote!

Lots of younger people are not enrolled to vote, while old people always vote. So remember to register and vote in the election this year.

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6, fig.height=3, dev='tikz'}
e <- read.csv("~/Downloads/electorate_default.csv", stringsAsFactors=FALSE)
library(ggplot2)
library(dplyr)
library(tidyr)
library(scales)
d <- e %>% mutate(Total = as.numeric(gsub(",","",Est.Eligible.Population)),
             Enrolled = as.numeric(gsub(",","",Total.Enrolled))) %>% select(Age, Total, Enrolled) %>% gather("Which", "Population", Total, Enrolled, convert=TRUE) %>%
  filter(Age != "Total") %>% mutate(Which = factor(Which, levels=c("Total", "Enrolled")))
ggplot(d) + geom_col(aes(x=Age, y=Population, fill=Which), position='dodge') +
  scale_y_continuous(labels=comma) + theme_bw(base_size=10) + theme(legend.position="top", legend.title=element_blank()) + scale_fill_manual(values=c("grey70", "grey30"))
```

If you aren't already on the electoral role (and are a permanent resident or citizen), the easiest way to make sure you can vote is to rock up to an advanced voting place (on concourse between 11th and 22nd September). You can register and place a special vote all within 10 minutes or so.

You can't register on election day, so make sure you do so beforehand!