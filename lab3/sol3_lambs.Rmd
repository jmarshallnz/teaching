...
Using `ggplot2` gives nicer results. The default is to do a scatterplot which isn't much use, but using `geom='boxplot'` converts to boxplots.
...
We can also facet on two variables at once, producing a grid of sex (rows) by rank (columns)

```{r}
qplot(Feed, BirthWeight, data=births, geom='boxplot') + facet_grid(Sex~Rank)
```

Our conclusion is there is an effect of feed, but it's only really noticeable once we first account for rank and sex. The HM feed treatment seems to result in the lowest weight lambs.
